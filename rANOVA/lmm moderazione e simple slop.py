...
# Carichiamo direttamente il file che contiene già tutti i compositi
...
def analisi_completa_moderazione(moderatore, output_file):
    ...
    for base_name, col_pre, col_post in pairs:
        ...
        try:
            ...
            if p_3vie < 0.05:
                # Pairwise tests
                ph = pg.pairwise_tests(data=long_df, dv='valore', within='tempo', between='gruppo', subject='ID', padjust='bonf')
...
analisi_completa_moderazione('BRIEF_TOT', os.path.join(dir_base, 'Analisi_Mod_BRIEF_TOT.xlsx'))
for sub in brief_subscales:
    out_file = os.path.join(dir_base, f'Analisi_Mod_{sub}.xlsx')
    analisi_completa_moderazione(sub, out_file)
analisi_completa_moderazione('ABAS_TOT', os.path.join(dir_base, 'Analisi_Mod_ABAS_TOT.xlsx'))
analisi_completa_moderazione('abas_suppongo_tot', os.path.join(dir_base, 'Analisi_Mod_ABAS_SUPPONGO_TOT.xlsx'))

# Moderatori Sottoscale
for sub in sottoscale_abas:
    # Standard
    analisi_completa_moderazione(f'ABAS_{sub}', os.path.join(dir_base, f'Analisi_Mod_ABAS_{sub}.xlsx'))
    # Suppongo (già create dall'altro file come abas_s_...)
    analisi_completa_moderazione(f'abas_s_{sub}', os.path.join(dir_base, f'Analisi_Mod_abas_s_{sub}.xlsx'))
