...
INPUT_XLSX = ROOT / "DATABASE DEFINITIVO.xlsx"
OUT_DIR = ROOT / "planning_minefield"
CLEAN_XLSX = OUT_DIR / "planning_minefield_database_pulito.xlsx"
RESULTS_XLSX = OUT_DIR / "planning_minefield_risultati.xlsx"
...
def run_prepost_ttests(df):
    ...
    for group_label, subset in [
        ("all", df),
        ("sperimentale", df[df["group_label"] == "sperimentale"]),
        ("controllo", df[df["group_label"] == "controllo"]),
    ]:
        rows.append(
            paired_test(
                subset,
...
def make_plot(
    ...
    post_col,
    path,
    ylabel="Differenza mattonelle",
    title=None,
    annotations=None,
    annotation_key=None,
):
    groups = [("sperimentale", 1), ("controllo", 2)]
    plot_data = []
    labels = []
    colors = []
    bracket_info = []
    ...
    ax.set_title(title or f"{task}: mattonelle percorse - mattonelle minime")
    ...
    fig.text(
        0.5,
        0.01,
        "Asterischi neri: q FDR < .05; asterischi rossi: p < .05 solo non corretto",
        ha="center",
        va="bottom",
        fontsize=8,
        color="#333333",
...
def make_all_prepost_plots(df, prepost_pairs, all_prepost_ttests):
    plot_dir = OUT_DIR / "grafici_prepost_tutte_variabili"
    plot_dir.mkdir(exist_ok=True)
    manifest = []
    annotations = annotation_lookup(all_prepost_ttests, "variable")

    for pair in prepost_pairs:
        filename = f"{safe_filename(pair['variable'])}_prepost_per_gruppo.png"
        ...
        make_plot(
            ...
            path,
            ylabel="Valore",
            title=f"{pair['variable']}: PRE vs POST per gruppo",
...
def write_outputs(
    ...
    try:
        ...
        results_path = OUT_DIR / f"{RESULTS_XLSX.stem}_aggiornato_{timestamp}.xlsx"
        write_results_workbook(
            results_path,
            descriptives,
            invalid_values,
...
def write_results_workbook(
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        descriptives.to_excel(writer, sheet_name="descrittive", index=False)
        invalid_values.to_excel(writer, sheet_name="valori_esclusi", index=False)
        ttests_pre_post.to_excel(writer, sheet_name="t_test_pre_post", index=False)
        all_prepost_ttests.to_excel(writer, sheet_name="t_test_pre_post_tutte", index=False)
        all_prepost_plots.to_excel(writer, sheet_name="grafici_tutte_variabili", index=False)
        prepost_without_group.to_excel(writer, sheet_name="prepost_senza_gruppo", index=False)
        prepost_with_group.to_excel(writer, sheet_name="prepost_con_gruppo", index=False)
        predictors_pre.to_excel(writer, sheet_name="predittori_pre", index=False)
        predictors_post.to_excel(writer, sheet_name="predittori_post", index=False)
        predictors_mix.to_excel(writer, sheet_name="predittori_mix_somma", index=False)
...
def main():
    ...
    make_plot(
        ...
        "planning_diff_pre",
        "planning_diff_post",
        OUT_DIR / "planning_diff_prepost_per_gruppo.png",
    ...
    make_plot(
        ...
        "pwm_diff_pre",
        "pwm_diff_post",
        OUT_DIR / "pwm_diff_prepost_per_gruppo.png",
        annotations=planning_pwm_annotations,
        annotation_key="PWM",
    )
    ...
    print(f"Clean database: {CLEAN_XLSX}")
