...
# Carica il database composito e prendi la colonna ID
composite_db = pd.read_excel(os.path.join(dir_base, '../descrittive/database_compositi.xlsx'))
if 'ID' in composite_db.columns:
    id_col = 'ID'
else:
    id_col = composite_db.columns[0]

# Funzione per riallineare i punteggi alle vere ID

def align_scores_with_id(scores, db, id_col):
    # Se l'indice dei punteggi è numerico, lo usiamo come posizione per prendere l'ID dal db
    if not scores.index.equals(db[id_col]):
        # Prova a riallineare per posizione
        scores = scores.copy()
        scores[id_col] = db[id_col].values[:len(scores)]
        scores = scores.set_index(id_col)
    return scores

# Carica i factor scores filtrati
...
# Riallinea tutti i punteggi alle ID originali
...
# Aggiungi suffisso per distinguere pre/post e dominio
...
# Unisci per indice (solo soggetti presenti in tutti)
df_scores = pd.concat([
...
# Aggiungi le variabili dal database composito
final_df = df_scores.join(composite_db.set_index(id_col), how='left')

# Salva il dataframe unico per LMM
out_scores_path = os.path.join(dir_base, 'scores_pca_unificato.xlsx')
final_df.to_excel(out_scores_path)
print(f"DataFrame unico dei punteggi PCA salvato in: {out_scores_path}")
