...
def write_summary(results: dict, elapsed_seconds: float) -> Path:
    ...
    lines = [
        ...
        f"- Covariate-adjusted group effects significant after FDR: {significant_count(training['covariate'])}",
        f"- PRE outcomes associated with age after FDR: {significant_count(pre_covariates, 'age_significant_fdr05')}",
        f"- PRE outcomes differing by sex after FDR: {significant_count(pre_covariates, 'sex_significant_fdr05')}",
